...
def measure_mean_signal_over_time(im_stack):
    ...
    max_loc = np.unravel_index(np.argmax(local_brightness), (cropped_height, cropped_width))
...
for i, (folder_path, folder_name, file_names) in enumerate(os.walk(root_folder)):
    ...
    if "aliquots" in folder_path:
        ...
        data_combined.at[i, "Temperature, °C"] = fetch_temperature_aliquoted(folder_path)
...
for i in data_combined.index:
    x = np.array(data_combined.at[i, "Time, min"])
    y = np.array(data_combined.at[i, "Mean signal, a.u."])
    y_err = np.array(data_combined.at[i, "STD signal, a.u."])
    label = data_combined.at[i, "DNA concentration"]
    plt.plot(x, y, label=label + " BTx32 DNA")
